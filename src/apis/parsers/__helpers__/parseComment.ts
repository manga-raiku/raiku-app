...
export function parseComment($comment: Cheerio<Element>, now: number) {
  const author = {
    ...
    avatar: $comment.find(".avartar-comment img").attr("src")!,
  ...
  const replies = $comment.find(".text-list-reply").length === 0 ? 0 : parseInt($comment.find(".text-list-reply").text().trim())
  const canDeleted = $comment.find(".remove_comnent").length > 0

  return { author, content, likes, time, replies, canDeleted }
}
