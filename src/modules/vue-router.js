...
function parseURL(parseQuery, location, currentLocation = "/") {
    let path; let query = {}; let searchString = ""; let hash = "";
...
const scrollTabPositions = new Map();
...
function saveScrollPositionTab(key, scrollPosition) {
    scrollTabPositions.set(key, scrollPosition);
}
function getSavedScrollPositionTab(key) {
    const scroll = scrollTabPositions.get(key);
    // consume it so it's not used again
    scrollTabPositions.delete(key);
    return scroll;
}
...
function createRouter(options) {
    ...
    function finalizeNavigation(toLocation, from, isPush, replace, data) {
        ...
        if (from.meta.isTab)
        saveScrollPositionTab(getScrollKey(from.fullPath,0), computeScrollPosition())
    ...
    function handleScroll(to, from, isPush, isFirstNavigation) {
        ...
        let scrollPosition = (!isPush && getSavedScrollPosition(getScrollKey(to.fullPath, 0))) ||
            ...
            null;
        if (to.meta.isTab &&!scrollPosition) scrollPosition =  getSavedScrollPositionTab(getScrollKey(to.fullPath, 0))
